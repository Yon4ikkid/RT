...
fn main() {
    ...
    let mut s: scene::Scene = scene::Scene { ambient_light_color: rendering::Color { r: AMB, g:AMB, b: AMB }, camera: c, objects: std::vec::Vec::new(),
                                            ...
                                            lightsource: Box::new(DirectionalLight {c: Color { r: 0.0, g: 0.0, b: 0.0 } , d: Vector { x: -0.0, y: 0.0, z: -1.0 }.unit()}) };
    ...
    let l1: Object = Object { m: Material { base_color: Color::new(1.0, 0.0, 1.0), refractive_index: 1.5, opacity: 1.0, roughness: 1.0 },
        s: Box::new(BiconvexLens::new(Vector::new(0.0, 50.0, 0.0), Vector::new(-1.0, -1.0, 1.0).unit(), 150.0, 0.02))};

    let par1: Object = Object { m: Material { base_color: Color::new(1.0, 0.0, 1.0), refractive_index: 1.5, opacity: 1.0, roughness: 0.0 },
        s: Box::new(Paraboloid::new(Vector::new(0.0, 130.0, 0.0), Vector::new(0.0, -1.0, 0.0).unit(), 30.0, 300.0))};
